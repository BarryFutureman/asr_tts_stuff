...
class RealtimeVoiceProcessor:
    def __init__(self):
        ...
        self.should_clear_buffer = False  # Flag to signal buffer clearing
    ...
    def flush(self):
        """Remove all fixed chunks from transcription and signal audio buffer clearing"""
        # Keep only non-fixed chunks
        self.full_running_transcription = [
            chunk for chunk in self.full_running_transcription
            if not chunk.is_fixed and False
        ]

        # Set flag to clear audio buffer on next processing cycle
        self.should_clear_buffer = True

        print("Flushed fixed chunks and flagged audio buffer for clearing")
    ...
    def is_garbage(self, text):
        """
        Detect if the generated text is garbage/filler that should be ignored.
        ...
        if word_count < 2:
            if (
                    text.strip().startswith("Yeah") or
                    text_lower.startswith("you") or
                    text.strip().startswith("Okay") or
                    text.strip().startswith("Good") or
                    text.strip().startswith("Oh")):
                return True

        if word_count < 4:
            if "thank you" in text_lower or text.strip().startswith("I"):
                return True
    ...
    def transcribe_audio(self, base64_audio):
        ...
        if not self.full_running_transcription:
        ...
        elif self.is_garbage(generated_text):
            # Remove silence
            pass
            ...
            if len(self.full_running_transcription) > 8:
                self.full_running_transcription[1].text = self.full_running_transcription[0].text + \
                                                          self.full_running_transcription[1].text
...
def format_messages_for_webui(msgs):
    formatted = []
    for msg in msgs[:]:
        content = msg.get("content", "")
        if isinstance(content, str):
            # Remove content between think tags (case insensitive)
...
def process_audio_streaming(audio_buffer, new_chunk, previous_text_display, chatbot_widget):
    sr, y = new_chunk

    # Check if we need to clear the buffer due to flush
    if realtime_audio_processor.should_clear_buffer:
        audio_buffer = None
        realtime_audio_processor.should_clear_buffer = False
        print("Cleared audio buffer due to flush")
    ...
    try:
        ...
        # Get chatbot messages and format for chatbot widget
        chatbot_messages = format_messages_for_webui(chatbot_instance.messages)

        return audio_buffer, processed_result.text, chatbot_messages
    except queue.Empty:
        return audio_buffer, previous_text_display, chatbot_widget


# Initialize processor
realtime_audio_processor = RealtimeVoiceProcessor()
chatbot_instance = Chatbot(realtime_voice_processor=None)
...
with gr.Blocks(title="Assistant") as demo:
    ...
    with gr.Tab("Chatbot"):
        ...
        chatbot_widget = gr.Chatbot(elem_id="chatbot", type="messages")

        audio_recorder.stream(
            fn=process_audio_streaming,
            inputs=[state, audio_recorder, markdown_display, chatbot_widget],
            outputs=[state, markdown_display, chatbot_widget]
        )
